...
import { filterRows, createRow, updateRow } from '../../shared/supabaseClient.ts';
...
export default async function (req) {
  try {
    ...
    if (action === 'login') {
      ...
      const users = await filterRows(base44, 'admin_users', { email, active: true });
    ...
    if (action === 'requestOtp') {
      ...
      await createRow(base44, 'otp_codes', { email, code, expires_at: expires, used: false });
      await base44.asServiceRole.integrations.Core.SendEmail({
        to: email,
        subject: 'KRD GROUP Admin — Verification Code',
    ...
    if (action === 'resetPassword') {
      ...
      const codes = await filterRows(base44, 'otp_codes', { email, used: false });
      ...
      await updateRow(base44, 'admin_users', users[0].id, { password: hashed });
      await updateRow(base44, 'otp_codes', valid.id, { used: true });
    ...
    if (action === 'createAdmin') {
      ...
      if (!email || !password) return Response.json({ error: 'Email and password required' }, { status: 400 });
      const existing = await filterRows(base44, 'admin_users', { email });
      if (existing && existing.length > 0) return Response.json({ error: 'Admin with this email already exists' }, { status: 409 });
      const hashed = await hashPassword(password);
      const created = await createRow(base44, 'admin_users', {
        email,
        name: name || 'Admin',
        password: hashed,
        role: role || 'admin',
